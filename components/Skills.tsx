...
const skills = [
  { name: 'React & Next.js', level: 92, color: '#00FFB2', category: 'Frontend', icon: 'FE' },
  { name: 'TypeScript', level: 85, color: '#00C8FF', category: 'Language', icon: 'TS' },
  { name: 'Hume AI & OpenAI', level: 80, color: '#FF6FB1', category: 'AI/ML', icon: 'AI' },
  { name: 'Tailwind + MUI', level: 90, color: '#B16FFF', category: 'Styling', icon: 'UI' },
  { name: 'Ionic React', level: 78, color: '#FFAA00', category: 'Mobile', icon: 'MB' },
  { name: 'Redux', level: 82, color: '#00FFB2', category: 'State', icon: 'ST' },
  { name: 'REST APIs', level: 88, color: '#00C8FF', category: 'Integration', icon: 'API' },
  { name: 'Git & Agile/Scrum', level: 86, color: '#B16FFF', category: 'Workflow', icon: 'WF' },
]
...
export default function Skills() {
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            const bar = (entry.target as HTMLElement).querySelector('.skill-bar') as HTMLElement | null
  ...
  return (
    <section id="skills" className="relative flex min-h-screen flex-col justify-center px-4 py-20 sm:px-6 sm:py-24 md:px-[6vw] lg:pr-[28vw] xl:pr-[32vw] xl:py-32 2xl:pr-[36vw]">
      <p className="mb-4 flex items-center gap-3 text-xs uppercase tracking-[0.22em]" style={{ color: 'var(--accent3)' }}>
          ...
          >
            ...
            <div className="mb-3 flex items-start justify-between gap-3">
              <div className="min-w-0">
                <span className="mr-2 inline-flex min-w-7 justify-center text-xs font-bold uppercase tracking-[0.22em]" style={{ color: skill.color }}>
                  {skill.icon}
                </span>
                <span className="font-syne text-sm font-bold" style={{ color: '#E2EEFF' }}>
                  {skill.name}
                </span>
              </div>
